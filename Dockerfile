...
# docker build . -t kauhdb:v1.5
# docker run -d -p 3000:3000 -e "ORIGIN=http://localhost:3000" --name kauhdb --mount source=kd-sqlite-db,target=/app/sqliteDB kauhdb:v1.5
# npx prisma migrate deploy
# run this comand in the container => npx prisma migrate dev --name mg1
# docker images
# docker save -o C:\docker_image_tars\kauhdb_v1.5.tar kauhdb:v1.5
# docker load -i kauhdb_v1.5.tar
...
# docker run -d --restart unless-stopped -p 3000:3000 -e "ORIGIN=http://172.30.5.190:3000"  -e "SECRET_ADMIN_PASSWORD=sys3211" --mount source=kd-sqlite-db,target=/app/sqliteDB --name kauh-dashboard-v1.5  kauhdb:v1.5
